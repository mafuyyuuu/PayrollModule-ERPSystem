...
export default function PayrollReports() {
    ...
    const fetchReportsData = async () => {
        try {
            ...
            const trendResponse = await fetch('http://localhost:8080/api/payroll/payroll-reports');
            if (trendResponse.ok) {
                ...
                trendData.forEach(p => {
                    ...
                    monthlyData[monthKey].total += parseFloat(p.net_pay || 0);
                    monthlyData[monthKey].count += 1;
                });
                setPayrollTrendData(Object.values(monthlyData).slice(-6));
            }
        } catch (error) {
    ...
    const handleExportPDF = async (reportType) => {
        ...
        try {
            ...
            if (reportType === 'Payroll Summary') {
                ...
                `;
            } else if (reportType === 'Payroll Trend') {
                htmlContent += `
                    <h2>Payroll Trend Report</h2>
                    <table>
                        <thead>
                            <tr>
                                <th>Month</th>
